...
import io.moquette.broker.server.ConnectionDescriptor;
import io.moquette.broker.server.netty.AutoFlushHandler;
import io.moquette.broker.server.netty.NettyUtils;
import io.moquette.broker.spi.ClientSession;
...
public class ProtocolProcessor {
    ...
    public void setBrokerMessageRepo(BrokerMessageRepo brokerMessageRepo) {
    ...
    public void processPubAck(Channel channel, PubAckMessage msg) {
        ...
        IMessagesStore.StoredMessage inflightMsg = m_sessionsStore.getInflightMessage(clientID, messageID);
    ...
    public void processPublish(Channel channel, PublishMessage msg) {
        ...
        String clientID = NettyUtils.clientID(channel);
        String topic = msg.getTopicName() + TopicType.PUSH.name();
        ...
        // FIXME: 2016/6/22(Enum.Lin) change moquette send content, after changeed. The content is messageId.
        ImMessage imMessage = persistImMessage(msg, clientID);
        ByteBuffer messageId = ByteBuffer.wrap(imMessage.getId().toString().getBytes());
        messageId.flip();
        msg.setPayload(messageId);
    ...
    private ImMessage persistImMessage(PublishMessage message, String fromId) {
        String topicName = message.getTopicName();
        String[] params = StringUtil.split(topicName, Delimiters.DIVIDE);
        ImMessage imMessage = new ImMessage();
        imMessage.setTopicName(topicName);
        imMessage.setAppId(params[0]);
        imMessage.setTenantId(params[1]);
        imMessage.setTopicOwnerType(TopicOwnerType.valueOf(params[2]));
        imMessage.setTargetId(Integer.parseInt(params[3]));
        imMessage.setInternal(true);
        imMessage.setFromId(fromId);
        imMessage.setMsgType(MsgType.PUSH);
        String payload = new String(message.getPayload().array());
        imMessage.setContent(payload);

        return brokerMessageRepo.insert(imMessage);
    }
...
//        m_interceptor.notifyClientDisconnected(clientID, username);
    ...
    public void processConnectionLost(String clientID, boolean sessionStolen, Channel channel) throws Exception{
